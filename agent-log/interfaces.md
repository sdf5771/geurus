...
- 포커스: 실행 시 포커스를 가져가지 않음(`show:false` → `showInactive()`)
- **그루 클릭 시 포커스 정책(`focusable`) — 보류 확정** [2026-09-16]
  - 1단계는 클릭해도 하는 일이 없어 지금 정할 필요가 없음. 현 상태(클릭 시 앱이 활성화됨) 유지
  - **2단계에서 "클릭하면 무엇을 할지"가 정해질 때 함께 결정**한다
  - 그때 qa가 실측할 것: `focusable:false`에서 클릭이 정상 수신되는지, 앱 활성화가 실제로 억제되는지 (현재는 **추정**)
  - 판단 재료: 클릭 목적이 5단계 Herdr pane 점프면 Geurus가 중간에 활성화될 필요가 없음(이중 전환) ↔ 4단계 드래그 그립·정원 패널 조작, 단축키·입력이 필요해지면 `focusable:false`는 뒤집어야 함
