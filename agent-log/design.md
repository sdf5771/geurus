...
1. **stretch 잎 겹침**: `LEAF_OFFSET.stretch.spread=-1`이 중심 옆 열(7↔8)을 맞바꿔 서로 다른 키가 같은 칸으로 모임 — `triple` (1,7), `stem` (2,7)·(2,8), `bud` (1,7)·(1,8). 결과: triple 가운데 잎 좌우 반전, bud 속 짙은 픽셀 축소. **이식 오류 아님**(원본 HTML과 바이트 동일). 선택지: A 그대로 / B Claude Design 원본 재검토 / C 그대로 + 정본 3-8에 "LEAF 패치는 키 삽입 순서, 나중 키 우선" 계약 명시
2. **정본 문구 차이** (구현 영향 없음, 정본은 사용자가 갱신)
   - "행 8~11 좌표 4종 동일" → 실제로 squash 행 11의 **1·12·14열** 다름(EXPR 칸은 동일)
   - "EXPR = 행 8~11" → sleepy z는 **행 4~5**
   - "표정 6종 · 288장" → 실제 **7종 · 336셀**
   - 정본·PM 브리프에 옛 파일명(위 표) · PM 브리프 7절 트레이 메뉴 "종료·설정"(1단계는 `종료`만 — 사용자 지시)
